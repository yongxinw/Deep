# Created by yongxinwang at 2019-11-29 16:30
# Created by yongxinwang at 2019-10-24 02:06
import torch
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, use_bn=True):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channels=3, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block=block, planes=64, blocks=layers[0], use_bn=use_bn)
        self.layer2 = self._make_layer(block=block, planes=128, blocks=layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], use_bn=use_bn)
        self.layer3 = self._make_layer(block=block, planes=256, blocks=layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], use_bn=use_bn)
        self.layer4 = self._make_layer(block=block, planes=512, blocks=layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], use_bn=use_bn)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, use_bn=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                norm_layer(planes * block.expansion, track_running_stats=False) if use_bn else nn.Identity(),
            )
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        # create pretrained_dict if the key in pretrained_dict is also in model_dict
